...
module.exports = {
    ...
    updatePart: function(req, res) {
        ...
        sails.models.parts.update({id: req.param('partId'), partName: req.body.partName}, part).exec(err => {
            if (err) {
    ...
    deletePart: function(req, res) {
        ...
        sails.models.parts.find(prt).exec((findErr, part) => {
            if (findErr) {
                res.send({ success: false, isError:true, message: 'Error in deleting part.', error: findErr });
            } else if (part.length === 0) {
                res.send({ success: false, isError:false, message: 'No Part with provided partId exist'});
            } else {
                sails.models.parts.destroy(part).exec(destroyErr => {
                    if (destroyErr) {
                        res.send({ success: false, isError:true, message: 'Error in deleting part.', error: destroyErr });
                    } else {
    ...
    isPartExist: function(req, res) {
        ...
        if (partId) {
            sails.models.parts.find({id: partId}).exec((err, part) => {
                if (err) {
                    res.send({ success: false, isError: true, message: 'Error occurred in fetching partId.', err });
                } else if (part.length === 0) {
                    res.send({ success: false, isError: false, message: 'Part you are looking for does not exist', err });
                } else {
                    res.send({ success: true, isError: false, message: 'Found part in our system', parts: part });
                }
            });
        } else {
            res.send({ success: false, isError: false, message: 'Cannot find part without "partId".' });
        }
    }
};
